...
class MyTestCase(unittest.TestCase):
    def test_detector(self):
        ...
        # shift the pmts in the local x direction
        if 1==2:
            x_offset = 25.5
            n_sensor = 5
            for i_sensor in range(n_sensor):
                i_str = str(i_sensor)
                photosensor_module_design['x_'+i_str].set_offset(x_offset)

        # shift the pmt modules in the detector x direction
        if 1==2:
            x_offset = 5.5
            n_module = len(my_detector.photo_sensor_modules)
            for i_module in range(n_module):
                i_str = str(i_module)
                detector_design['x_'+i_str].set_offset(x_offset)
